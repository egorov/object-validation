describe('maxLength', () => {
  ...
  it('should return an error', () => {
    ...
    expect(validate('echo', 3)).toEqual({it: 'maximum length', is: 3});
    expect(validate('echo', 2)).toEqual({it: 'maximum length', is: 2});
    expect(validate(array, 2)).toEqual({it: 'maximum length', is: 2});
    expect(validate(array, 1)).toEqual({it: 'maximum length', is: 1});
    expect(validate({}, 1)).toEqual({it: 'maximum length', is: 1});
    expect(validate(true, 2)).toEqual({it: 'maximum length', is: 2});
    expect(validate(35, 1)).toEqual({it: 'maximum length', is: 1});
  });
